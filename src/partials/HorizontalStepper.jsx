...
const HorizontalStepper = forwardRef((props, ref) => {
  ...
  // FIXED: Simplified step determination logic
  const determineCurrentStep = (participantData, seragamData, isCompleteStatus) => {
    if (!participantData) return 1;

    const { submission_status, is_complete } = participantData;

    // If participant is accepted or on measurement AND complete is true
    if ((submission_status === 'accepted' || submission_status === 'on_measurement') && isCompleteStatus) {
      return steps.length; // Go to last step (Pengukuran Seragam)
    }
    
    // If participant is complete but not accepted/on measurement
    if (is_complete && submission_status !== 'accepted' && submission_status !== 'on_measurement') {
      return steps.length - 1; // Go to Status step
    }
    
    // If participant has uniform data and is accepted/on measurement
    if (seragamData && seragamData.length > 0 && (submission_status === 'accepted' || submission_status === 'on_measurement')) {
      return steps.length; // Go to last step
    }

    // Default: use the current step from props or maintain current
    return props.currentStep || currentStep;
  };
  ...
  useEffect(() => {
    ...
    // if (nstep) {
    //   setCurrentStep(parseInt(nstep));
    //   localStorage.setItem('lastActiveStep', currentStep.toString());
    // } else {
      
    // }
  }, [currentStep, nstep, navigate]);
  ...
  // FIXED: Main useEffect with proper step determination
  useEffect(() => {
    if (props.applicant?.length > 0) {
      console.log('applicant', props.applicant[0].participants[0]);
      setApplicant(props.applicant[0]);
      ...
      if (props.applicant[0].participants.length > 0) {
        const currentParticipant = props.applicant[0].participants[0];
        setParticipant(currentParticipant);

        // Determine the correct current step based on participant status
        const determinedStep = determineCurrentStep(
          currentParticipant, 
          dataSeragam, 
          complete
        );
        
        console.log('Determined step:', determinedStep, 'Complete:', complete, 'Submission status:', currentParticipant.submission_status);
        
        setCurrentStep(determinedStep);
        localStorage.setItem('lastActiveStep', determinedStep.toString());

        getStatus();
        setEdit(currentParticipant.is_draft);

        // Load existing data
        if (currentParticipant.participant_father_data.length > 0) {
          setDataAyah(currentParticipant.participant_father_data[0]);
        }
        if (currentParticipant.participant_mother_data?.length > 0) {
          setDataIbu(currentParticipant.participant_mother_data[0]);
        }
        if (currentParticipant.participant_wali_data.length > 0) {
          setDataWali(currentParticipant.participant_wali_data[0]);
        }

        getFatherData(currentParticipant.id);
        getMotherData(currentParticipant.id);
        getWaliData(currentParticipant.id);
        getParticipantDocuments(currentParticipant.id);
        getSeragamData(currentParticipant.id);
        getSchoolUniformModel(props.applicant[0].applicant_schools[0].schools.school_id);

        const dataStudentCategory = {
          student_category: currentParticipant.student_category,
          updated_at: currentParticipant.updated_at
        };
        setApplicantStudentCategory(dataStudentCategory);
        ...
        const dataMetodeUangPangkal = {
          metode_uang_pangkal: currentParticipant.metode_uang_pangkal
        };
        setDataMetodeUangPangkal(dataMetodeUangPangkal);
      ...
      if (props.applicant[0].applicant_orders.length > 0) {
        setApplicantOrder({
          invoice_number: props.applicant[0].applicant_orders[0].invoice_number,
          status: props.applicant[0].applicant_orders[0].status
        });
      }
    }
    ...
    if (nstep) {
      setTimeout(() => {
        ...
        if (pid) {
          const stepHandlers = {
            2: () => { getApplicantData(props.applicant[0].id); getParticipantData(pid); },
            3: () => getFatherData(pid),
            4: () => getMotherData(pid),
            5: () => getWaliData(pid),
            6: () => getParticipantDocuments(pid),
            7: () => getParticipantData(pid),
            8: () => getParticipantData(pid)
          };
          
          if (stepHandlers[nstep]) {
            stepHandlers[nstep]();
          }
        }
      }, 500);
    ...
    if (complete && (participant?.submission_status === 'accepted' || participant?.submission_status === 'on_measurement')) {
      setCurrentStep(steps.length);
      localStorage.setItem('lastActiveStep', steps.length.toString());
    }
  }, [props.applicant, nstep, complete, participant?.submission_status]);
  ...
  const getSeragamData = async (id) => {
    ...
    // If uniform data exists and participant is accepted/on measurement, go to last step
    if (data && data.length > 0 && (participant?.submission_status === 'accepted' || participant?.submission_status === 'on_measurement')) {
      setCurrentStep(steps.length);
      setComplete(true);
      localStorage.setItem('lastActiveStep', steps.length.toString());
  ...
  // FIXED: Improved getComplete function
  const getComplete = async (value) => {
    const { data, error } = await supabase.from('participants')
      .update({
        is_complete: value,
    ...
    if (data) {
      ...
      // FIXED: Automatically adjust step when completion status changes
      if (value && (participant.submission_status === 'accepted' || participant.submission_status === 'on_measurement')) {
        setCurrentStep(steps.length);
        localStorage.setItem('lastActiveStep', steps.length.toString());
      }
    }
  ...
  // FIXED: Improved stepper display logic
  const getStepDisplay = (index) => {
    const stepNumber = index + 1;
    
    // For the last step (Pengukuran Seragam)
    if (stepNumber === steps.length) {
      // Show tick if complete AND (participant is accepted/on measurement OR has uniform data)
      if (complete && (participant?.submission_status === 'accepted' || 
          participant?.submission_status === 'on_measurement' || 
          dataSeragam?.length > 0) && dataSeragam && dataSeragam.length >0) {
        return <TiTick size={24} />;
      }
      return stepNumber;
    }
    
    // For all other steps, show tick if step is completed
    if (stepNumber < currentStep || complete) {
      return <TiTick size={24} />;
    }
...
//     }
  ...
  // ... rest of your existing functions (getFatherData, getMotherData, getWaliData, etc.)
  // Keep all your existing data fetching and form handling functions
  ...
  const form = [
    <Pembayaran scroll={scroll} applicantOrder={applicantOrder} />,
    <IdentitasForm onSubmit={getIdentitas} dataApplicant={applicant} dataParticipant={participant} isPending={isPending} loading={loading} setParamNavigasi={setParamNavigasi} edit={edit} complete={complete} currentStep={currentStep} />,
    <DataAyahForm onSubmit={getDataAyah} dataAyah={dataAyah} isPending={isPending} loading={loading} setParamNavigasi={setParamNavigasi} edit={edit} complete={complete} currentStep={currentStep} setComplete={setComplete} />,
    <DataIbuForm onSubmit={getDataIbu} isPending={isPending} loading={loading} complete={complete} setParamNavigasi={setParamNavigasi} edit={edit} currentStep={currentStep} setCurrentStep={setCurrentStep} setComplete={setComplete} dataIbu={dataIbu} />,
    <DataWaliForm onSubmit={getDataWali} isPending={isPending} loading={loading} complete={complete} setParamNavigasi={setParamNavigasi} edit={edit} currentStep={currentStep} setCurrentStep={setCurrentStep} setComplete={setComplete} dataWali={dataWali} />,
    <BerkasForm onSubmit={getDataBerkas} retrieveData={getParticipantDocuments} dataBerkas={dataBerkas} participant={participant.id?participant.id:participant_id} school={applicantSchool.id} isPending={isPending} loading={loading} setParamNavigasi={setParamNavigasi} edit={edit} complete={complete} currentStep={currentStep} setComplete={setComplete} />,
    <VerifikasiKeluargaForm onSubmit={getDataVerifikasiKeluarga} dataVerifikasiKeluarga={dataVerifikasiKeluarga} dataBerkas={dataBerkas} student_category={participant.student_category} retrieveData={getDataVerifikasiKeluarga} isPending={isPending} loading={loading} complete={complete} setParamNavigasi={setParamNavigasi} edit={edit} currentStep={currentStep} setComplete={setComplete}/>,
    <MetodeUangPangkal onSubmit={getDataMetodeUangPangkal} dataApplicant={applicantSchool} dataMetodeUangPangkal={dataMetodeUangPangkal} dataApplicantCategory={applicantStudentCategory} isPending={isPending} loading={loading} complete={complete} setParamNavigasi={setParamNavigasi} edit={edit} currentStep={currentStep} setComplete={setComplete}/>,
    <Status onSubmit={getStatus} participant={participant} applicant={applicant} school="" dataStatus={dataStatus} complete={complete} setParamNavigasi={setParamNavigasi} currentStep={currentStep} getCurrentStep={getCurrentStep} scrollToStep={scrollToStep} getEdit={getEdit} getComplete={getComplete}/>,
    <PengukuranSeragam onSubmit={getPengukuranSeragam} participant={participant_id?participant_id:participant.id} school={applicantSchool.id} gender={applicant.gender} ref={ref} dataSeragam={dataSeragam} schoolUniformModel={schoolUniformModel} isPending={isPending} loading={loading} complete={complete} setParamNavigasi={setParamNavigasi} currentStep={currentStep} getCurrentStep={getCurrentStep} getEdit={getEdit} getComplete={getComplete}/>
  ];
  ...
  return (
    <>
      <div className="flex items-center justify-center shadow-md p-4">
        ...
        {/* FIXED: Stepper Container with improved logic */}
        <div ref={stepperRef} className="flex overflow-x-auto space-x-4 scrollbar-hide">
          {steps.map((step, index) => (
            <div 
              key={index}
              className={`flex-shrink-0 w-64 p-6 bg-white rounded-lg shadow-md flex items-center justify-center ${
                ...
                // (index + 1 === steps.length && complete && 
                //  (participant?.submission_status === 'accepted' || 
                //   participant?.submission_status === 'on_measurement'))) 
                  // ? "bg-green-50 complete" : ""
              }`}
            >
              <div className={`step flex items-center justify-center w-8 h-8 rounded-full ${
                 ( index + 1 <= steps.length-1 && index === 0 || index + 1 < currentStep && complete)? <TiTick size={24} /> : index+1 == steps.length && (dataSeragam && dataSeragam.length >0) ? <TiTick size={24} /> : index + 1} 
                `}>
      ...
      {/* FIXED: Navigation buttons with improved logic */}
      <div className='flex-wrap justify-center px-5'>
        <div className=''>
          <div className="w-full">
            <button
            ...
            >
              {isPending || loading ? (
                <div className="inline-block h-8 w-8 animate-[spinner-grow_0.75s_linear_infinite] rounded-full bg-current align-[-0.125em] text-primary opacity-0 motion-reduce:animate-[spinner-grow_1.5s_linear_infinite]" role="status">
                  <span className="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]">Menyimpan...</span>
                </div>
              ) : (
                currentStep === steps.length ? "Selesai" : "Lanjut"
              )}
            ...
            <div className="w-full">
              <button
                ...
                onClick={() => {
                  ...
                  // Handle data refresh for specific steps
                  const stepRefreshHandlers = {
                    6: () => getParticipantDocuments(participant_id || participant.id),
                    7: () => {
                      getParticipantData(participant_id || participant.id);
                    ...
                    8: () => getParticipantData(participant_id || participant.id)
                  };
                  
                  if (stepRefreshHandlers[currentStep]) {
                    stepRefreshHandlers[currentStep]();
                  }
