...
const useRegister = () => {
  ...
  // Determine which form to use based on whether we're in edit mode
  const formMethods = useForm({
    resolver: zodResolver(isEditMode ? editSchema : registerSchema),
    defaultValues: defaultRegisterValues,
  });

  // Effect to set edit mode when pid is present in form values
  useEffect(() => {
    const subscription = formMethods.watch((value) => {
      setIsEditMode(!!value.pid);
    });
  ...
  const { mutate: onChangeSchool, isPending: isChangeSchoolPending } = useMutation({
    mutationFn: (data) => {
      setPhoneNumber(data.phone_number);
      setFullName(data.full_name);
      ...
      return AuthService.change_school(
        ...
        data.full_name,
        data.media,
        data.password_ || '', // Handle optional password
        data.phone_number,
        data.regist_number || data.pid, // Use pid if regist_number not available
        data.school_id,
        data.subschool
      );
    },
    ...
    onError: (error) => {
      setError(error);
      console.log('Perubahan jenjang error:', error);
    },
  });
...
- Mohon simpan nomor ini untuk mendapatkan update informasi -`
    ...
    try {
      const response = await axios.post("/api/auth/send-notif", 
        { message: data, type: 'form-success', token: null },
  ...
  return {
    ...
    results,
    form: formMethods,
    loading: isRegisterPending || isChangeSchoolPending,
    error, 
    notified,
    isEditMode
  };
